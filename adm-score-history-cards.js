...
const periodos=[['Dia','dia'],['Semana','semana'],['Mês','mes']];
...
function pontosPossiveis(p,ini,fim,dados){
...
function pontosGanhos(p,ini,fim,dados){
  const hist=dados.historico.filter(h=>pertence(h,p)).map(h=>({...h,data:h.data||h.dataExecucao||''}));
  return hist.filter(h=>h.data>=iso(ini)&&h.data<=iso(fim)).reduce((s,h)=>s+(Number(h.pontosGanhos)||0),0);
}
function faixa(p,ini,fim,dados){return {ganho:pontosGanhos(p,ini,fim,dados),max:pontosPossiveis(p,ini,fim,dados)};}
function resumo(p,ref,dados){
  const s=inicioSemana(ref),e=fimSemana(ref),mi=inicioMes(ref),mf=fimMes(ref);
  return {dia:faixa(p,ref,ref,dados),semana:faixa(p,s,e,dados),mes:faixa(p,mi,mf,dados)};
}
...
function mover(input,delta){if(!input)return;const d=parseISO(input.value||iso(new Date()));d.setDate(d.getDate()+delta);input.value=iso(d);input.dispatchEvent(new Event('change',{bubbles:true}));}
function garantirNavegacao(input,id){
  if(!input||document.getElementById(id))return;
  const nav=document.createElement('div');nav.id=id;nav.className='rf-period-nav';nav.innerHTML='<button type="button" data-prev="1">◀ Dia anterior</button><button type="button" data-hoje="1">Hoje</button><button type="button" data-next="1">Próximo dia ▶</button>';
  input.parentElement?.appendChild(nav);
  nav.querySelector('[data-prev]')?.addEventListener('click',()=>mover(input,-1));
  nav.querySelector('[data-next]')?.addEventListener('click',()=>mover(input,1));
  nav.querySelector('[data-hoje]')?.addEventListener('click',()=>{input.value=iso(new Date());input.dispatchEvent(new Event('change',{bubbles:true}));});
}
...
async function renderDashboard(){
  ...
  const icones={dia:'☀️',semana:'📅',mes:'🗓️'};
  host.innerHTML=`<div class="rf-score-title"><h3>📊 Acompanhamento de pontuação</h3><span style="font-size:12px;color:#64748b">Referência: ${ref.toLocaleDateString('pt-BR')}</span></div>`+(perfis.map(p=>{const r=resumo(p,ref,dados);return `<div class="rf-score-person"><strong>${esc(p.nome||'Integrante')}</strong><div class="rf-score-grid">${periodos.map(([rotulo,chave])=>card(chave==='dia'&&iso(ref)===iso(new Date())?'Hoje':chave==='dia'?ref.toLocaleDateString('pt-BR'):rotulo,r[chave],icones[chave])).join('')}</div></div>`;}).join('')||'<p>Nenhum integrante encontrado.</p>')+`<div class="rf-period-note">Os máximos são calculados pelas tarefas programadas em cada período. Ao escolher uma data antiga, Dia, Semana e Mês representam aquele período histórico completo.</div>`;
}
