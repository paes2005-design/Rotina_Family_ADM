...
const test1755FixKey=g=>`rf_adm_test_1755_fix_${ACTIVE_DATE}_${clean(g).toUpperCase()}`;
...
async function activate1755(groupId){
  ...
  if(localStorage.getItem(test1755FixKey(groupId))==='1')return false;
  ...
  try{
    ...
    if(!target){
      localStorage.setItem(test1755FixKey(groupId),'1');
    ...
    const now=new Date();
    const start=clean(target.horaSugeridaInicio)||'17:55';
    const dados={
      status:'Em andamento', horarioInicio:start, inicioExecutadoEm:isoAt(start,0), dataExecucao:ACTIVE_DATE,
      iniciouComAtraso:false, atrasoInicioMin:0, inicioAntecipado:false, antecipacaoMin:0,
      motivoInicioAntecipado:'', tipoMotivoInicioAntecipado:'', toleranciaConsumidaMin:0, toleranciaConsumidaSeg:0,
      testeTecnicoTravaUnica:true, testeTecnicoTravaUnicaEm:now.toISOString(),
      compensacaoTecnicaInicio:true, compensacaoTecnicaInicioMotivo:'teste_trava_unica_1755', compensacaoTecnicaInicioEm:now.toISOString()
    };
    const exec={
      grupoId:groupId, perfilId:clean(target.perfilId), perfilNome:clean(target.perfilNome), tarefaId:target.id,
      tarefaGrupoId:clean(target.tarefaGrupoId), nomeTarefa:clean(target.nome), data:ACTIVE_DATE, diaSemana:ACTIVE_DAY,
      horaSugeridaInicio:start, horaSugeridaFim:clean(target.horaSugeridaFim), ...dados
    };
    const batch=writeBatch(db);
    batch.update(doc(db,'tarefas',target.id),dados);
    batch.set(doc(db,'execucoes',`${ACTIVE_DATE}__${target.id}`),exec,{merge:true});
    await batch.commit();
    localStorage.setItem(test1755Key(groupId),'1');
    localStorage.setItem(test1755FixKey(groupId),'1');
    log('teste.trava_unica_1755_inicio_compensado',{grupoId:groupId,tarefaId:target.id,nomeTarefa:clean(target.nome),inicio:start});
    alert(`Teste corrigido: “${clean(target.nome)}” permanece em andamento, mas o início foi compensado para ${start}.\n\nAtraso de início: 0.`);
    setTimeout(()=>location.reload(),600);
    return true;
  }catch(error){
    test1755Running=false;
    console.error('Teste 17:55:',error);
    alert('Não foi possível corrigir o início da tarefa das 17:55.');
    return false;
  }
}
