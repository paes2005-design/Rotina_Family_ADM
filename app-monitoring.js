...
const PENDING_JUSTIFICATION_KEY = '__rfJustificationPendingLogsV1';
...
const pendingJustificationLogs = Array.isArray(window[PENDING_JUSTIFICATION_KEY]) ? window[PENDING_JUSTIFICATION_KEY].splice(0) : [];
window[PENDING_JUSTIFICATION_KEY] = [];
for (const item of pendingJustificationLogs) window.rotinaLog(item.evento || 'justificativa.log_pendente', item.detalhes || {}, item.nivel || 'info');
window.dispatchEvent(new CustomEvent('rotina-monitoring-ready',{detail:{app:APP_KIND,monitorVersion:MONITOR_VERSION,pendingJustificationLogs:pendingJustificationLogs.length}}));
