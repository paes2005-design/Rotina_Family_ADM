...
function decorarTarefas(){
  ...
  document.querySelectorAll('[data-family-task-id]').forEach(el=>{
    ...
    if(!btn){btn=document.createElement('button');btn.type='button';btn.className='family-task-alarm-admin';btn.style.cssText='margin:5px 0 0 8px;padding:7px 9px;border:1px solid #f87171;border-radius:8px;background:#fff;color:#991b1b;font-size:12px;font-weight:900;line-height:1.1;cursor:pointer;vertical-align:middle;flex:0 0 auto;white-space:nowrap;max-width:100%';btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();abrir(tarefaDoElemento(el))});alvo.appendChild(btn)}
    atualizarBotao(btn,tarefa.tarefaId,tarefa.dataAgendada);
  });
}
function atualizarBotao(btn,id,dataAgendada){const a=alarmeDaTarefa(id,dataAgendada),texto=a?.ativo?'🔒 Alarme':'⏰ Programar',titulo=a?.ativo?`Alarme de ${formatarDataBR(dataAgendada)} ativado pelo ADM`:`Programar alarme para ${formatarDataBR(dataAgendada)}`;if(btn.textContent!==texto)btn.textContent=texto;if(btn.title!==titulo)btn.title=titulo;btn.setAttribute('aria-label',titulo);btn.style.background=a?.ativo?'#fee2e2':'#fff'}
...
function boot(){zerarViradaSemana();decorarTarefas();let agendado=false;const solicitarDecoracao=()=>{if(agendado)return;agendado=true;requestAnimationFrame(()=>{agendado=false;decorarTarefas()})};new MutationObserver(solicitarDecoracao).observe(document.body,{childList:true,subtree:true});window.addEventListener('rotina-family-tasks-rendered',solicitarDecoracao);setInterval(decorarTarefas,5000);setInterval(zerarViradaSemana,30000)}
